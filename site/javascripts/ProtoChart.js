...
Proto.Chart = Class.create({
	...
	processGraphData: function() {
	    ...
	    if(this.options.pies.show) {return; } // shouldn't be necessary for pie graphs at all!
	    this.supplemental = false;
	    if(this.graphData.find(function(gd) { return gd.supplemental == true; }))
		this.supplemental = true; // invert cons if this is supplemental style

	    this.graphData.each(function(gd) {
		    var data = gd.data;
		    gd.maxyvalue = 0; // a local indicator of max value
//		window.console.log(gd.data);		
...
//			    this.yaxis.datamin = y;
			if(!this.supplemental || gd.filter == 'pro') {
			    if (y > this.yaxis.datamax)
				this.yaxis.datamax = y;
			} else if(this.supplemental && gd.filter == 'con') {
			    if(-y < this.yaxis.datamin)
				this.yaxis.datamin = -y;
			}
...
//		window.console.log(gd.data);
	    ...
	    if(this.supplemental) { // equalize x & y for supplemental graphs.
		if(this.yaxis.datamax > -this.yaxis.datamin) {
		    this.yaxis.datamin = -this.yaxis.datamax;
		}else {
		    this.yaxis.datamax = -this.yaxis.datamin;
		}
	    }
		this.graphData.each(function(gd) {
		    var data = gd.data;
		    var scaleMultiplier = 1;
		    if(!this.supplemental || gd.filter == 'pro') { // scaleMultiplier for nonsupplemental or pro supplemental (the top side of the graph)
			scaleMultiplier = this.yaxis.datamax/gd.maxyvalue;
		    } else if(this.supplemental && gd.filter == 'con') { // scaleMultiplier for supplemental con (the bottom side of the graph)
			scaleMultiplier = this.yaxis.datamin/gd.maxyvalue; // this will build in the negative component (as we're diving datamin by maxyvalue)
		    }
		    gd.scaleMultiplier = scaleMultiplier;
		    data.each(function(d){
			if(d == null) {
			    return;
			}
	...
	setSpacing: function() {
	    ...
	    // added code for chart offsets via right-hand labels.
	    this.chartOffset.right += this.yLabelMaxWidth + this.options.grid.labelMargin;
	...
	insertLabels: function() {
		...
		this.xaxis.ticks.each(function(tick){
			if (!tick.label || tick.v < this.xaxis.min || tick.v > this.xaxis.max)
				return;
		    html += '<div style="position:absolute;top:' + (this.chartOffset.top + this.chartHeight + this.options.grid.labelMargin) + 'px;left:' + Number((this.chartOffset.left + this.translateHoz(tick.v) - this.xLabelBoxWidth/2)).toFixed(0) + 'px;width:' + this.xLabelBoxWidth + 'px;text-align:center" class="tickLabel">' + tick.label + "</div>";
...
//		this.supplemental = true; // invert cons if this is supplemental style
		...
		this.yaxis.ticks.each(function(tick){
			if (!tick.label || tick.v < this.yaxis.min || tick.v > this.yaxis.max)
				return;
//		    window.console.log(this.graphData);
		    var pos = 0; // 0 for left, 1 for right.
		    this.graphData.each(function(gd) {
//			window.console.log(gd);
//			window.console.log(this.supplemental);
			var color = gd.color;
			var filter = gd.filter;
			var supplementalData = gd.supplemental;
			var scaleMultiplier = gd.scaleMultiplier;
			var data = gd.data;
			var rightSide = this.canvasWidth - this.yLabelMaxWidth;
			var skip = false;
			var fontWeight = 'normal';
			var left = 0;
			if(gd.filter == 'pro' && !this.supplemental) {
			} else if(gd.filter == 'pro' && this.supplemental && !supplementalData) {
			    if(Number(tick.label) < 0) { skip = true; }
			} else if(gd.filter == 'pro' && supplementalData) {
			    var left = rightSide;
			    fontWeight = 'heavy';
			    if(Number(tick.label) < 0) { skip = true; }
			} else if(gd.filter == 'con' && !this.supplemental) { // if no supplemental -graph-, put the second graph on the right.
			    var left = rightSide;
			} else if(gd.filter == 'con' && this.supplemental && !supplementalData) { // if supplemental graph but this isn't supplemental data, put on left side in negative.
			    if(Number(tick.label) > 0) { skip = true; }
			} else if(gd.filter == 'con' && supplementalData) {
			    var left = rightSide;
			    fontWeight = 'heavy';
			    if(Number(tick.label) > 0) { skip = true; }
			}
			if(!skip) {
			    html += '<div id="ylabels" style="color:'+ color+ ';font-weight: '+ fontWeight+ 
			    ';position:absolute;top:' + Number((this.chartOffset.top + this.translateVert(tick.v) - this.yLabelMaxHeight/2)).toFixed(0)+
			    'px;left:' + left +  'px;width:' + this.yLabelMaxWidth +
			    'px;text-align:right" class="tickLabel">' + Number(Number(tick.label)/scaleMultiplier).toFixed(0) + "</div>";
			}
		    }.bind(this));
		}.bind(this));
	 ...
	 */
    plotLine: function(data, offset, scaleMultiplier) {
        var prev, cur = null, drawx = null, drawy = null;
	var minSeparation = (1/12) + .01;
        ...
        for (var i = 0; i < data.length; ++i) {
            ...
            var x1 = Number(prev[0]), y1 = Number(prev[1]) * scaleMultiplier,
            x2 = Number(cur[0]), y2 = Number(cur[1]) * scaleMultiplier;
	    ...
	    // ensure that imaginary 0 points are plotted
	    if (x2 - x1 > minSeparation*2) { // a long gap
//		window.console.log("x1: " + x1 + ", x1+minSep: " + (x1+minSeparation) + ", x2-minSep: " + (x2-minSeparation) + ", x2: " + x2);
		this.context.lineTo(this.translateHoz(x1+minSeparation), this.translateVert(0) + offset);
		this.context.lineTo(this.translateHoz(x2-minSeparation), this.translateVert(0) + offset);
	    } else if (x2 - x1 > minSeparation) { // a quick jump down.
//		window.console.log("x1: " + x1 + ", x1+minSep: " + (x1+minSeparation) + ", x2: " + x2);
		this.context.lineTo(this.translateHoz(x1+minSeparation), this.translateVert(0) + offset);
	    }
//	    } else { // normal drawing route
...
//	    }
	 ...
	 */
    plotLineArea: function(data, scaleMultiplier) {
        var prev, cur = null;
	...
	var minSeparation = (1/12) + .01;
	    ...
	    // ensure that imaginary 0 points are plotted
	    // plotLineArea doesn't have 'offset' for y -- ?
	    if (x2 - x1 > minSeparation*2) { // a long gap
//		window.console.log("x1: " + x1 + ", x1+minSep: " + (x1+minSeparation) + ", x2-minSep: " + (x2-minSeparation) + ", x2: " + x2);
		this.context.lineTo(this.translateHoz(x1), this.translateVert(y1));
		this.context.lineTo(this.translateHoz(x1+minSeparation), this.translateVert(0));
		this.context.lineTo(this.translateHoz(x2-minSeparation), this.translateVert(0));
...
//		window.console.log("x1: " + x1 + ", x1+minSep: " + (x1+minSeparation) + ", x2: " + x2);
	...
	var scaleMultiplier = graphData.scaleMultiplier;
        ...
        if (sw > 0) {
            ...
            this.plotLine(graphData.data, lw/2 + sw/2 + this.context.lineWidth/2, scaleMultiplier);

            this.context.lineWidth = sw / 2;
            this.context.strokeStyle = "rgba(0,0,0,0.2)";
            this.plotLine(graphData.data, lw/2 + this.context.lineWidth/2, scaleMultiplier);
        }
        ...
        if (graphData.lines.fill) {
            ...
            this.plotLineArea(graphData.data, scaleMultiplier);
        }

        this.plotLine(graphData.data, lw/2, scaleMultiplier);
        this.context.restore();
    },
	/**
	 * function: plotPoints
	...
	insertLegend: function() {
		...
		this.graphData.each(function(gd, index){
			...
			var label = gd.label;
		    var value = gd.data[0][1];
		    if(this.options.legend.labelFormatter != null) {
			    label = this.options.legend.labelFormatter(label, value);
		    }
	...
	onMouseDown: function(e) {
		...
		this.selectionInterval = setInterval(this.updateSelectionOnMouseMove.bind(this), 50);
...
//	    window.console.log('onClick');
	...
	onSelectionMouseUp: function(e) {
		...
		this.domObj.fire("ProtoChart:mouseup", [ ]);
...
//				if(xabs < xsens && yabs < ysens && (xabs+yabs) < n.dist){
			    if(xabs < xsens && xabs < n.dist) {
				n.dist = (xabs);
				n.x = data[j][0];
				n.y = data[j][1];
				n.scaleMultiplier = this.graphData[i].scaleMultiplier;
		...
		if(n.mouse && n.mouse.track && !this.prevHit || (this.prevHit && n.x != this.prevHit.x && n.y != this.prevHit.y)){
			...
			if(n.x !== null && n.y !== null){
			    ...
			    el.innerHTML = n.mouse.trackFormatter({x: n.x.toFixed(decimals), y: (n.y/n.scaleMultiplier).toFixed(decimals)});
